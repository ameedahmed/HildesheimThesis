...
def save_attention_analysis(image_path, model, config, output_base_dir, args):
    ...
    if parent_folder in labels:
        ...
        for label_name, idx in labels.items():
            clean_parent = parent_folder.replace('_ogdataset', '').replace('_resizeddataset', '').replace('_preprocessed_384_dataset', '').replace('train_', '')
            clean_label = label_name.replace('_ogdataset', '').replace('_resizeddataset', '').replace('_preprocessed_384_dataset', '').replace('train_', '')
            
            if clean_label.lower() == clean_parent.lower():
                true_label_idx = idx
                break
            elif clean_label.lower() in clean_parent.lower() or clean_parent.lower() in clean_label.lower():
                true_label_idx = idx
                break
    
    if true_label_idx is None:
        true_label_idx = 0  # Default to first class
    ...
    if high_attention_coords:
        coords_df = pd.DataFrame(high_attention_coords)
        coords_df['image_name'] = base_filename
        coords_df['true_class'] = label_list[true_label_idx]
        coords_df['predicted_class'] = label_list[predicted_class_index]
        coords_df['is_correct'] = is_correct_classification
        coords_df['confidence'] = confidence
        coords_df['attention_type'] = 'global'
        
        coords_df.to_csv(os.path.join(output_dir, f'HighAttentionCoordinates_{base_filename}.csv'), index=False)
    
    # Save all attention coordinates
    all_coords = []
    ...
    all_coords_df = pd.DataFrame(all_coords)
    all_coords_df.to_csv(os.path.join(output_dir, f'AllAttentionCoordinates_{base_filename}.csv'), index=False)
