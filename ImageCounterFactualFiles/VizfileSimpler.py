...
def extract_attention_coordinates(attention_map):
    """Extract top-k attention coordinates from the attention map."""
    # Flatten the attention map and get top-k indices
    ...
    for idx in range(len(flat_map)):
        y, x = divmod(idx, w)
        attention_value = flat_map[idx]
        coords.append((x, y, attention_value))
    return coords
...
def load_model():
    ...
    # Model configuration
    model_path = f"{base_path}/dataset_resized_train_val_test_combi/TrainValTestModelResult/finetune/20250726-093555-CSWin_96_24322_base_384-384/model_best.pth.tar" ##CHANGE MODEL PATH HERE
    num_classes = 6
    img_size = 384
...
def load_labels():
    """Load class labels."""
    labels_path = f"{base_path}/dataset_resized_train_val_test_combi/trainvaltestcombi_artstyle_class_index.json" ##CHANGE LABELS PATH HERE
    with open(labels_path, 'r') as f:
        temp_label = json.load(f)
    
    labels = {}
    label_list = []
    for i in range(6):  # num_classes #####CHANGE THIS TO NUMBER OF CLASSES
        full_path = temp_label[str(i)][0]
        folder_name = os.path.basename(full_path)
        style_name = folder_name.replace("train_", "").replace("_preprocessed_384_dataset", "") ###ALSO CHANGE THIS TO THE CORERCT CLASS NAME
        labels[style_name] = i
...
def process_image(image_path, model, device, labels, label_list, output_dir):
    ...
    true_label_idx = labels.get(parent_folder, 0) ##CHANGE THIS TO THE CORRECT TRUE LABEL INDEX
    
    # Generate visualizations
    vis_global, raw_global = generate_visualization(
        model, transform(img), 384, class_index=predicted_class_index
    )
    # Extract attention coordinates
    attention_coords = extract_attention_coordinates(raw_global)
    # Save results
    ...
    ## Write the attention coordinates, filename, true label, predicted label, x, y and attention value and if the prediction is correct to a csv file
    csv_path = f"{base_path}/ImageCounterfactualExplanations/NewAttentionMap/Ukiyo/Ukiyo_attention_coordinates.csv" ##CHANGE CSV PATH HERE
    if not os.path.exists(csv_path):
        print(f"Creating new csv file: {csv_path}")
        with open(csv_path, 'w', newline='') as csvfile:
            csv_writer = csv.writer(csvfile)
            csv_writer.writerow(['filename', 'true_label', 'predicted_label', 'x', 'y', 'attention_value', 'is_correct'])
            for coord in attention_coords:
                csv_writer.writerow([base_filename, label_list[true_label_idx], label_list[predicted_class_index], coord[0], coord[1], coord[2], true_label_idx == predicted_class_index])
    else:
...
def main():
    # Configuration
    test_data_dir = f"{base_path}/dataset_resized_train_val_test_combi/test/train_Ukiyo_preprocessed_384_dataset" ##CHANGE TEST DATA DIR HERE /data/vifapi/ameed_ahmed_thesis/HildesheimThesis/CSWin_Transformer_main/dataset_resized_train_val_test_combi/test/train_Ukiyo_preprocessed_384_dataset
    output_dir = f"{base_path}/ImageCounterfactualExplanations/NewAttentionMap/Ukiyo" ##CHANGE OUTPUT DIR HERE
