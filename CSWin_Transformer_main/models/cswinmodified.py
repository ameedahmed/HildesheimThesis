...
class LePEAttention(nn.Module):
    ...
    def forward(self, qkv,register_hook=False, return_attentions=False):
        """
        x: B L C
        """
...
class CSWinBlock(nn.Module):
    ...
    def forward(self, x,register_hook=False, return_attentions=False):
        ...
        if self.branch_num == 2 and not return_attentions:
            x1 = self.attns[0](qkv[:,:,:,:C//2])
            x2 = self.attns[1](qkv[:,:,:,C//2:])
            attened_x = torch.cat([x1,x2], dim=2)
        
        elif self.branch_num == 2 and return_attentions:
            x1, attn_vertical = self.attns[0](qkv[:,:,:,:C//2], register_hook=register_hook, return_attentions=True)
            x2, attn_horizontal = self.attns[1](qkv[:,:,:,C//2:], register_hook=register_hook, return_attentions=True)            
            attened_x = torch.cat([x1,x2], dim=2)
            
        
        elif self.branch_num == 1 and not return_attentions:
            attened_x = self.attns[0](qkv)
        else:
            attened_x, attn_global = self.attns[0](qkv, register_hook=register_hook, return_attentions=True)
        ...
        if self.branch_num==1 and not return_attentions:
            return x
        elif self.branch_num == 1 and return_attentions:
            return x, attn_global
        elif self.branch_num == 2 and not return_attentions:
            return x
        else:
            return x, attn_vertical, attn_horizontal
...
class CSWinTransformer(nn.Module):
    ...
    def forward(self, x,register_hook=False, return_attentions=False):
        ...
        for blk in self.stage1: ##work with stage numbers instead of branches, at stage 4, employ global layer
            if not return_attentions:
                if self.use_chk:
                ...
                x,attn_vertical,attn_horizontal = blk(x, register_hook=register_hook, return_attentions=True)
                attention_vertical.append(attn_vertical)
                attention_horizontal.append(attn_horizontal)
                overall_attn.append(attn_vertical)
                overall_attn.append(attn_horizontal)

#Global For Stage 4                    x, attn_global = blk(x, register_hook=register_hook, return_attentions=True)
#Global For Stage 4                    attn_global_list.append(attn_global)
            ...
            for blk in blocks:
                if not return_attentions:
                    ...
                    if blocks!=self.stage4:
                        ...
                        x, attn_global = blk(x, register_hook=register_hook, return_attentions=True)
                        attn_global_list.append(attn_global)
                        overall_attn.append(attn_global)
        ...
        if not return_attentions:
            ...
            return self.head(x), attn_global_list, attention_vertical, attention_horizontal,overall_attn
